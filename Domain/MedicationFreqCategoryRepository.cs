...
{
    public class MedicationFreqCategoryRepository : BaseRepository
    {
        public MedicationFreqCategoryRepository(dbfactory db) : base(db) { }
        ...
        public override JArray GetListJointImp(int pageSize, int pageIndex)
        {
            ...
            return _db.GetArray(@"
select ID,Code,Value,ValueMessage,IsActive 
from data_medicationfreqcategory 
where IsDeleted=0
LIMIT ?p1,?p2
", offset,pageSize);
        }
...
", id);
        ...
        public override Dictionary<string, object> GetValue(JObject data)
        {
            ...
            dict["Value"] = data["value"]?.ToObject<string>();
            dict["ValueMessage"] = data["valuemessage"]?.ToObject<string>();
            return dict;
        }
